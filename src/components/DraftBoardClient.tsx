...
export default function DraftBoardClient({ initialProspects }: { initialProspects: PlayerSummary[] }) {
  ...
  return <div className="space-y-7">
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 space-y-4">
      <div suppressHydrationWarning className="flex flex-col lg:flex-row gap-3"><div className="relative flex-1"><Search className="w-4 h-4 absolute left-3 top-3 text-slate-500" /><input suppressHydrationWarning data-protonpass-ignore="true" data-1p-ignore value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search player, team, or position" className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-9 pr-3 py-2.5 text-sm" /></div><select value={team} onChange={(e) => setTeam(e.target.value)} className="bg-slate-950 border border-slate-800 rounded-xl px-3 py-2.5 text-sm"><option value="ALL">All organizations</option>{teams.map((name) => <option key={name}>{name}</option>)}</select><div className="flex bg-slate-950 border border-slate-800 rounded-xl p-1"><button onClick={() => setView("cards")} className={`p-2 rounded-lg ${view === "cards" ? "bg-slate-800 text-emerald-400" : "text-slate-500"}`}><LayoutGrid className="w-4 h-4" /></button><button onClick={() => setView("table")} className={`p-2 rounded-lg ${view === "table" ? "bg-slate-800 text-emerald-400" : "text-slate-500"}`}><List className="w-4 h-4" /></button></div></div>
